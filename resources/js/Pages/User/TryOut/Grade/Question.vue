...
import { wrapLatexInText } from "../../TryOut/ExamGroup/wrapLatex.js";
...
export default {
    ...
    setup(props) {
        ...
        const processedQuestion = computed(() => {
            return wrapLatexInText(props.question?.question || "");
        });

        const processedOptions = computed(() => {
            const opts = {};
            if (!props.question) return opts;
            (props.questionLists[props.indexPage]?.answer_order || "")
                .split(",")
                .forEach((ans) => {
                    opts[ans] = wrapLatexInText(
                        props.question["option_" + ans] || ""
                    );
                });
            return opts;
        });

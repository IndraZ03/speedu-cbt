...
class AccountBalanceRepository extends BaseRepository implements AccountBalanceInterface
{
    ...
    public function update($attributes, $id)
    {
        ...
        if ($attributes->transaction_status == "paid") {
            ...
            $message = "*[TRANSAKSI SUKSES " . (strtoupper($setting->app_name ?? "isi data setting terlebih dahulu")) . "]*\n\n*TOP UP SALDO* \n\nNomor Transaksi: " . $accountBalance->code . "\nTotal Pembayaran: *Rp" . number_format($accountBalance->top_up_balance, 2, ",", ".") . "*\n\n*SELAMAT, SALDO ANDA TELAH BERTAMBAH*.\n\n*Pembelian Paket TRY OUT*\n1. Masuk ke dashboard (dengan melakukan klik bagian kiri)\n2. Pilih paket yang anda inginkan dengan melakukan scroll ke bawah dan klik Pilih Paket\n3. Lakukan pembayaran menggunakan metode Saldo Akun\n\n*Pembelian TRY OUT*\n1. Masuk ke dashboard (dengan melakukan klik bagian kiri)\n2. Pilih kotak merah paling atas bertuliskan try out sekdin\n3. Pilih TRY OUT yang akan dibeli (Misalkan SKD) klik selengkapnya\n4. Pilih Try Out yang akan dibeli dengan klik \"Beli Try Out\"\n\nterimakasih.";
